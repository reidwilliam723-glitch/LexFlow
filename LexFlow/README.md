...
LexFlow is a **Windows typing assistant**. It sits in the system tray and suggests words, spelling fixes, and grammar fixes while you type in other apps (Notepad, Word, browsers, and most text fields).
...
| Item | What it is |
| --- | --- |
| **[INSTALL.md](INSTALL.md)** | How to start LexFlow (no extra software) |
| **[GUIDE.md](GUIDE.md)** | How to use suggestions, grammar, rewrite, and Settings |
| **`Run`** | Double-click **`LexFlow.exe`** here |
...
Testers now install once from **`LexFlow-win-Setup.exe`** and the app updates itself after that. To cut a release, bump `<Version>` in `LexFlow.Settings\LexFlow.Settings.csproj`, then from this folder:
...
That refreshes **`Run\LexFlow.exe`**. Portable builds do **not** auto-update. Do not ask testers to install .NET.
