...
public class UpdatePolicyTests
{
    ...
    public void DetectsNewerVersionAndFormatsNotes()
    {
        ...
        var manifest = UpdatePolicy.ParseManifest("""
            {"version":"2.1.0","releaseNotes":"Privacy guard on rewrite.","downloadUrl":"https://example.com/lexflow.zip","sha256":"abc","privacyNotes":"Rewrite no longer sends blocked fields."}
            """);
...
public class PersonalizationExportTests
{
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "lexflow-test-" + Guid.NewGuid().ToString("N"));
