...
/// <summary>
/// Marker to identify LexFlow's own synthetic input, so KeyboardListener can filter it out.
/// This prevents feedback loops where our injected keystrokes get re-processed.
/// </summary>
internal static class InputMarkers
{
    internal const uint LexFlowInjectedMarker = 0x4C465753; // "LFWS" in hex
}
            ...
            var input = new INPUT
            {
                ...
                u = new InputUnion
                {
                    ki = new KEYBDINPUT
                    {
                        ...
                        time = 0,
                        dwExtraInfo = new IntPtr(InputMarkers.LexFlowInjectedMarker)
                    }
                }
            };
            inputs.Add(input);
