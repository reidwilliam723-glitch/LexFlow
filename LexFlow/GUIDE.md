...
LexFlow runs in the background and helps while you type in other apps.

## Tray icon

Right-click the LexFlow icon next to the clock:
...
**Ctrl+Shift+Z** undoes the last LexFlow insert/edit (not the app’s own Undo in every program).
...
LexFlow should **not** suggest in password boxes. It also skips some remote/admin tools.
...
LexFlow checks GitHub for new versions on startup; **no typing data is included in that check**. It only asks GitHub "what is the newest version?"

If a new version exists, LexFlow downloads it in the background and then asks whether to restart. Nothing is installed until you answer **Yes**. You can also check any time from the tray icon → **Check for updates…**, and turn the startup check off in Settings → General.
...
7. Exit from the tray, start `Run\LexFlow.exe` again  
...
- Is the tray icon present? If not, start `Run\LexFlow.exe` again.
- Double-press Ctrl (it may be toggled off).
- Try Notepad first; some apps block injected keys.
- Run as a normal user. If the target app is **Run as administrator** and LexFlow is not, Windows can block typing into that app.
