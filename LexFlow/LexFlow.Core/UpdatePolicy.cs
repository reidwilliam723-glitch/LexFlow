...
public static class UpdatePolicy
{
    ...
    public static string FormatNotification(UpdateCheckResult result)
    {
        var notes = string.IsNullOrWhiteSpace(result.ReleaseNotes)
            ? "Open the download page when you are ready — LexFlow will not install this by itself."
        ...
        return $"LexFlow {result.LatestVersion} is available (you have {result.CurrentVersion}). {notes}{privacy}{verified}";
    }
