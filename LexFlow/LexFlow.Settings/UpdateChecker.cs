...
{
    /// <summary>
    /// Repository that hosts the LexFlow releases consumed by Velopack. The
    /// packaging script (Settings\release.ps1) uploads to this same repo.
    /// </summary>
    private const string RepositoryUrl = "https://github.com/reidwilliam723-glitch/LexFlow";
    ...
    private static async Task RunAsync(Action<Action> invokeOnUi, Action requestShutdown, bool interactive)
    {
        ...
        try
        {
            ...
            if (!manager.IsInstalled)
            {
                ...
                {
                    Report(invokeOnUi,
                        "This copy of LexFlow was not installed with the LexFlow installer, so it cannot update itself.\n\n" +
            ...
            if (update == null)
            {
                if (interactive)
                {
                    Report(invokeOnUi, $"LexFlow {manager.CurrentVersion} is up to date.");
                }
            ...
            invokeOnUi(() =>
            {
                var answer = MessageBox.Show(
                    $"LexFlow {update.TargetFullRelease.Version} is ready to install. Restart now?",
                    "Update Available",
