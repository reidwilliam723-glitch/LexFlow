...
public class SuggestionOverlay : ISuggestionOverlay
{
    ...
    public const string WordClassName = "LexFlowSuggestionOverlay";
    public const string GrammarClassName = "LexFlowGrammarOverlay";
    ...
    private const uint WM_LEXFLOW_SHOW = WM_APP + 1;
    private const uint WM_LEXFLOW_HIDE = WM_APP + 2;
    private const uint WM_LEXFLOW_REPAINT = WM_APP + 3;
    private const uint WM_LEXFLOW_MOVE = WM_APP + 4;
    ...
    public SuggestionOverlay(OverlayThemeHost themeHost)
        : this(themeHost, WordClassName, "LexFlow Suggestions", null)
    {
    }
    ...
    public void ShowSuggestions(IEnumerable<Suggestion> suggestions, int x, int y, int lineHeight = 20)
    {
        ...
        if (_windowHandle != IntPtr.Zero)
        {
            ...
            PostMessage(_windowHandle, WM_LEXFLOW_SHOW, (IntPtr)commandId, IntPtr.Zero);
        }
    }
    ...
    public void ReplaceSuggestions(IEnumerable<Suggestion> suggestions)
    {
        ...
        if (_windowHandle != IntPtr.Zero)
        {
            PostMessage(_windowHandle, WM_LEXFLOW_REPAINT, IntPtr.Zero, IntPtr.Zero);
        }
    }
    ...
    private void ForceHideWindow()
    {
        ...
        SendMessage(_windowHandle, WM_LEXFLOW_HIDE, (IntPtr)commandId, IntPtr.Zero);
    }
    ...
    public void MoveTo(int x, int y, int lineHeight = 20)
    {
        ...
        if (_windowHandle != IntPtr.Zero)
        {
            PostMessage(_windowHandle, WM_LEXFLOW_MOVE, IntPtr.Zero, IntPtr.Zero);
        }
    }
