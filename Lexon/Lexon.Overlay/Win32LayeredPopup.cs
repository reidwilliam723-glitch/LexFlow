...
public abstract class Win32LayeredPopup : IDisposable
{
    ...
    protected const uint WM_LEXON_SHOW = WM_APP + 1;
    protected const uint WM_LEXON_HIDE = WM_APP + 2;
    protected const uint WM_LEXON_REPAINT = WM_APP + 3;
    ...
    protected void Repaint()
    {
        if (WindowHandle != IntPtr.Zero)
        {
            PostMessage(WindowHandle, WM_LEXON_REPAINT, IntPtr.Zero, IntPtr.Zero);
        }
    }
    ...
    protected void RequestShow(int x, int y, int width, int height)
    {
        ...
        if (WindowHandle != IntPtr.Zero)
        {
            PostMessage(WindowHandle, WM_LEXON_SHOW, IntPtr.Zero, IntPtr.Zero);
        }
    }
    ...
    public virtual void Hide()
    {
        ...
        SendMessage(WindowHandle, WM_LEXON_HIDE, IntPtr.Zero, IntPtr.Zero);
    }
