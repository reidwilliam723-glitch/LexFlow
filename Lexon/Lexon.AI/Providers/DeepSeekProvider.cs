...
/// <summary>
/// DeepSeek-powered suggestion provider
/// </summary>
public class DeepSeekProvider : IAIProvider
{
    ...
    public DeepSeekProvider(string? apiKey = null, HttpClient? httpClient = null, AIResponseCache? cache = null, string? model = null)
    {
        _apiKey = apiKey ?? Environment.GetEnvironmentVariable("DEEPSEEK_API_KEY") ?? string.Empty;
        if (string.IsNullOrEmpty(_apiKey))
        {
            throw new InvalidOperationException("DeepSeek API key is required. Set DEEPSEEK_API_KEY environment variable or pass apiKey parameter.");
        }
    ...
    public async Task<IEnumerable<Suggestion>> GetSuggestionsAsync(TextContext context, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var response = await CallDeepSeekAsync(prompt, cancellationToken);
